...
Theme Theme::dark()
{
    ...
    t.stepDisabled = juce::Colour (0xff232630);
...
Theme Theme::light()
{
    ...
    t.stepDisabled = juce::Colour (0xffebedf1);
                                          ...
                                          bool isHighlighted, bool isDown)
{
    ...
    juce::Colour fill = on ? t.accent : t.panelAlt;
    ...
    g.setColour (on ? t.accent.darker (0.2f) : t.outline);
...
void DYLookAndFeel::drawComboBox (juce::Graphics& g, int w, int h, bool, int, int, int, int, juce::ComboBox& box)
{
    ...
    const float ax = static_cast<float> (w) - 12.0f, ay = h / 2.0f;
    juce::Path arrow;
...
{
    return uiFont (juce::jmin (13.0f, buttonHeight * 0.55f));
}

juce::Font DYLookAndFeel::getComboBoxFont (juce::ComboBox& box)
{
    return uiFont (juce::jmin (13.0f, box.getHeight() * 0.6f));
}
